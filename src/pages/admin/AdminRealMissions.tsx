...
export default function AdminRealMissions() {
  const [missions, setMissions] = useState<RealMission[]>([]);
  const [loading, setLoading] = useState(true);
  const [isAdding, setIsAdding] = useState(false);
  const [formData, setFormData] = useState<Partial<RealMission>>({});
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-black text-navy uppercase tracking-widest">Real Missions</h1>
        <button
      ...
      {isAdding && (
        <form onSubmit={handleSave} className="bg-white p-8 rounded-xl shadow-lg border border-line mb-8 grid md:grid-cols-2 gap-6">
          ...
          <button type="submit" className="bg-navy text-white py-3 rounded-lg font-bold md:col-span-2 hover:bg-[#051b2e] transition-colors">Save Mission</button>
        </form>
      )}

      <div className="grid sm:grid-cols-2 gap-6">
        {missions.map(m => (
          <div key={m._id} className="bg-white rounded-xl shadow border border-gray-100 overflow-hidden flex flex-col">
            <div className="h-48 bg-gray-100 relative">
              <img src={m.image} alt={m.title} className="w-full h-full object-cover" />
            </div>
            <div className="p-5 flex flex-col flex-grow">
              <h3 className="font-bold text-navy text-xl mb-1">{m.title}</h3>
              <p className="text-sm text-gltOrange font-semibold mb-3">{m.date}</p>
              <p className="text-gray-600 text-sm line-clamp-3 mb-4 flex-grow">{m.description}</p>
              
              <div className="flex justify-between items-center mt-auto pt-4 border-t border-gray-50">
                <span className="text-xs text-gray-400">Added: {new Date(m.createdAt).toLocaleDateString()}</span>
                <button onClick={() => handleDelete(m._id)} className="text-red-500 hover:text-red-700 transition">
                  <Trash2 size={18} />
                </button>
              </div>
            </div>
